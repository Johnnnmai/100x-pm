...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="100X PM API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "100x-pmApiUrl",
                  String(config.100x-pmApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "100x-pmApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://100x-pm.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "100x-pm"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. 100X PM persists a device key during onboarding so pairing approvals
