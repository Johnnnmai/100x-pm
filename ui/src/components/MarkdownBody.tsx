...
import { parseProjectMentionHref } from "@100x-pmai/shared";
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`100x-pm-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="100x-pm-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("100x-pm-mermaid-status", error && "100x-pm-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="100x-pm-mermaid-source">
...
export function MarkdownBody({ children, className }: MarkdownBodyProps) {
  ...
  return (
    <div
      className={cn(
        "100x-pm-markdown prose prose-sm max-w-none break-words overflow-hidden prose-pre:whitespace-pre-wrap prose-pre:break-words prose-code:break-all",
            ...
            if (parsed) {
              ...
              return (
                <a
                  ...
                  className="100x-pm-project-mention-chip"
