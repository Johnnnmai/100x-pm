...
Environment overrides:
  ...
  PC_DATA=/tmp/data           100X PM data dir to use
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/100x-pm-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/100x-pm-data}"
...
env \
  ...
  pnpm 100x-pmai onboard --yes --data-dir "$PC_DATA"
