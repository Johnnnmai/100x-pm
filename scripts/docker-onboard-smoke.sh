...
IMAGE_NAME="${IMAGE_NAME:-100x-pm-onboard-smoke}"
...
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@100x-pm.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-100x-pm-smoke-password}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      ...
      'timeout 20s npx --yes "100x-pmai@${PAPERCLIPAI_VERSION}" auth bootstrap-ceo --data-dir "$PAPERCLIP_HOME" --base-url "$PAPERCLIP_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/100x-pm-onboard-smoke.XXXXXX")"
