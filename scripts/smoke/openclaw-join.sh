...
SMOKE_IMAGE="${SMOKE_IMAGE:-100x-pm-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-100x-pm-openclaw-smoke}"
...
assert_status "200"
if ! grep -q "100X PM OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.100x-pm.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
