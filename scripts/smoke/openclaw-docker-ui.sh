...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.100x-pm-openclaw.override.yml"
...
100x-pm_base_url="$(detect_100x-pm_base_url || true)"
...
100X PM URL for OpenClaw container:
EOF
  if [[ -n "$100x-pm_base_url" ]]; then
    cat <<EOF
  $100x-pm_base_url
...
EOF
  ...
  If 100X PM rejects the host, run on host machine:
    pnpm 100x-pmai allowed-hostname host.docker.internal
  Then restart 100X PM and re-run this script.
EOF
  fi
  cat <<EOF
