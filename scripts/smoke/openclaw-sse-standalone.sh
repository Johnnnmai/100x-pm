...
OPENCLAW_USER="${OPENCLAW_USER:-100x-pm-smoke}"
...
Run your 100X PM heartbeat procedure now.
EOF
...
PAYLOAD="$(jq -nc \
  ...
  '{
    ...
    metadata: {
      ...
      100x-pm_session_key: ("100x-pm:run:" + $runId)
    }
  }')"
...
args=(
  ...
  -H "x-openclaw-session-key: 100x-pm:run:${PAPERCLIP_RUN_ID}"
  -D "$headers_file"
  -o "$body_file"
  --data "$PAYLOAD"
  "$OPENCLAW_URL"
