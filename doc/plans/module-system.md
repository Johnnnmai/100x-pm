# 100X PM Module System

## Overview

100X PM's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends 100X PM's API, UI, and data model | Yes |
...
modules/
  observability/
    100x-pm.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@100x-pmai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@100x-pmai/core";
...
```
1. Scan modules/ directory for 100x-pm.module.json manifests
...
```jsonc
// 100x-pm.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@100x-pmai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@100x-pmai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:100x-pm/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.100x-pm.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm 100x-pmai store list                    # browse available modules and templates
pnpm 100x-pmai store install <module-id>     # install a module
pnpm 100x-pmai store import <template-id>    # import a company template
pnpm 100x-pmai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one 100X PM instance | `server:started` |
...
4. **Module config** — `100x-pm.config.json` with per-module config, env var interpolation
...
10. **`@100x-pmai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
