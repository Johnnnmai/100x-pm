...
Agents need API keys to authenticate with 100X PM. The current approach
...
**Trust model:** The adapter process runs on the same machine as the 100X PM
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** 100X PM generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a 100X PM CLI
...
```
100x-pm auth login
# Opens browser -> user confirms -> token stored at ~/.100x-pm/credentials
```
...
managed by the 100X PM server itself.
...
1. A company admin (user or agent) generates an **invite URL** from 100X PM.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The 100X PM SKILL.md (or a link to it)
   - What information 100X PM needs from the agent (e.g. webhook URL, adapter
...
5. 100X PM stores the pending registration.
...
7. On approval, 100X PM provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** 100X PM issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.100x-pm.ing/skills/100x-pm/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How 100X PM should send you heartbeats",
...
1. Generate a 100X PM invite link for the company.
...
4. A 100X PM company member approves the new agent.
5. 100X PM begins sending heartbeats to the OpenClaw webhook endpoint.
...
| **P3**   | CLI auth flow                     | `100x-pmai auth login` for developer-managed remote agents.                                      |
