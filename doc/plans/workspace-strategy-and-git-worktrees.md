...
`PAP-447` asks how 100X PM should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same 100X PM instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into 100X PM:

- some operators use 100X PM to manage 100X PM and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as 100X PM
- Claude and Codex expose different built-in affordances, so 100X PM should not overfit to one tool

## Core Product Decision

100X PM should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by 100X PM
...
- 100X PM should not assume that every adapter can see or use a host filesystem path directly
...
If 100X PM makes worktrees agent-first, it will blur:
...
- 100X PM core resolves and records execution workspace state
...
That gives 100X PM room to support:
...
- 100X PM handles workspace runtime services under the hood using project/workspace policy
...
Once 100X PM is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that 100X PM cannot infer
...
- when issue moves to done, should 100X PM auto-commit?
- should it auto-push?
...
100X PM already has the right foundation for a project-first model.
...
For server-spawned local adapters, 100X PM already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that 100X PM should mirror directly

Implication:

- for `codex_local`, 100X PM should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but 100X PM should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, 100X PM should separate:
...
100X PM should be able to express intentions such as:
...
These are not specific to 100X PM. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".100x-pm/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a 100X PM-specific `devServer` flag.
...
100X PM should distinguish between:
...
100X PM should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, 100X PM should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to 100X PM
...
For runtime services specifically, 100X PM should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as 100X PM-only dev-server flags
...
3. Inject resolved execution workspace details into `context.100x-pmWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / 100X PM instance can be reused through env/config injection when appropriate
...
- 100X PM can decide whether to reuse or start a fresh service deterministically
...
3. Test against 100X PM-on-100X PM workflows first.
...
7. Workspace runtime services are modeled generically, not as 100X PM-specific dev-server toggles.
