...
This document defines the V1 design for a 100X PM adapter that integrates with
...
4. synthesized stdout events for 100X PM UI/CLI
...
3. **Agent callback auth to 100X PM** uses a bootstrap exchange flow (no long-lived 100X PM key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from 100X PM endpoints, not full SKILL.md prompt injection.
...
- `100x-pmPublicUrl` (optional override; else `PAPERCLIP_PUBLIC_URL` env)
...
1. `adapterConfig.100x-pmPublicUrl`
...
Goal: avoid putting long-lived 100X PM credentials in prompt text.

Flow:

1. Before launch/follow-up, 100X PM mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. 100X PM validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all 100X PM API calls.
...
1. Prompt includes a compact instruction to fetch skills from 100X PM.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/100x-pm`
   - `GET /api/skills/100x-pm-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <100x-pmPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active 100X PM run context.
...
5. `100x-pmPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from 100X PM skill endpoints |
| 100X PM auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from 100X PM context.
3. Add richer PR metadata surfacing in 100X PM UI.
