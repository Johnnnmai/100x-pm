...
Add a single storage subsystem for 100X PM that supports:
...
- API serves attachment bytes through authenticated 100X PM endpoints.
...
- Default local path is under instance root: `~/.100x-pm/instances/<instanceId>/data/storage`.
...
- `100x-pmai onboard` writes a valid `storage` config block by default.
- `100x-pmai configure --section storage` can switch between local and s3 modes.
...
- `100x-pmai doctor` reports actionable storage status.
