...
1. Start 100X PM in auth mode.
```bash
cd <100x-pm-repo-root>
...
3. In 100X PM UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in 100X PM UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from 100X PM invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in 100X PM with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in 100X PM UI that new issue exists.

10. Watch logs during test (optional but helpful):
```bash
docker compose -f /tmp/openclaw-docker/docker-compose.yml -f /tmp/openclaw-docker/.100x-pm-openclaw.override.yml logs -f openclaw-gateway
```
