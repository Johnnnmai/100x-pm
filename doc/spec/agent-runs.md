...
This spec defines how 100X PM actually runs agents while staying runtime-agnostic.
...
1. 100X PM is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, 100X PM manages lifecycle, and on exit 100X PM parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `100x-pm.skill` (shared 100X PM skill text block)
