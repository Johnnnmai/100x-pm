...
100X PM V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a 100X PM-shaped design.

Assumption for this document: 100X PM is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class 100X PM objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into 100X PM.

The main conclusion is:

- 100X PM should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- 100X PM should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- 100X PM should use multiple extension classes instead of one generic plugin bag:
...
If 100X PM does this well, the examples you listed become straightforward:
...
Relevant 100X PM files reviewed for current extension seams:
...
It is too dangerous for 100X PM core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for 100X PM — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
100X PM should absolutely do this.
...
100X PM should copy this discipline.
...
100X PM should adopt this style for plugin actions, automations, and UI schemas.
...
100X PM can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for 100X PM too.

## What 100X PM Should Not Copy Directly
...
100X PM is a control plane for an operator-managed instance with company objects.
...
100X PM is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

100X PM should not auto-load arbitrary code from a workspace repo like `.100x-pm/plugins` or project directories.
...
For 100X PM, equivalent hooks into:
...
100X PM should not allow plugins to silently replace:
...
For 100X PM it would be risky because it combines:
...
100X PM should require an explicit operator install step.

## Why 100X PM Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | 100X PM |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted 100X PM instance |
...
That means 100X PM should borrow the good ideas from `opencode` but use a stricter architecture.

## 100X PM Already Has Useful Pre-Plugin Seams

100X PM has several extension-like seams already:
...
This is good news.
100X PM does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended 100X PM Plugin Model
...
100X PM already has this pattern implicitly:
...
- `plugin` means integration code that talks to 100X PM through a typed plugin protocol and capability model
...
- invoke explicit 100X PM actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@100x-pmai/plugin-sdk/ui`) with design tokens and common components.
...
100X PM should treat plugin installation as a global instance-level action.

Examples:

- install `@100x-pm/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over 100X PM objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

100X PM already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for 100X PM too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@100x-pm/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@100x-pmai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running 100X PM instance.

A `create-100x-pm-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how 100X PM achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the 100X PM server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. 100X PM's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@100x-pmai/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@100x-pmai/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For 100X PM

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@100x-pmai/plugin-sdk";

export default definePlugin({
  id: "@100x-pm/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit 100X PM entities
    });
    ...
    ctx.events.on("plugin.@100x-pm/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@100x-pmai/plugin-sdk/ui";
...
Any 100X PM plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific 100X PM objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@100x-pm/plugin-linear`)
...
- reuse core tables when the data is clearly part of 100X PM itself
...
If a concept is becoming part of 100X PM's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of 100X PM itself
...
### 3. Scope plugin data to 100X PM objects before adding custom schemas

A lot of plugin data naturally hangs off existing 100X PM objects:
...
If 100X PM eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@100x-pm/plugin-workspace-files`
...
Package idea: `@100x-pm/plugin-terminal`
...
Package idea: `@100x-pm/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving 100X PM.
- Board reviews the diff after a run without leaving 100X PM.
...
- optional `events.emit` (e.g. `plugin.@100x-pm/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@100x-pm/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@100x-pm/plugin-linear`

This plugin syncs 100X PM work with Linear. It is useful for:

- importing backlog from Linear
- linking 100X PM issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between 100X PM and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into 100X PM.
- 100X PM issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@100x-pm/plugin-github-issues`

This plugin syncs 100X PM issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to 100X PM issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with 100X PM task governance
...
- Plugin settings:
  ...
  - whether new 100X PM issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active 100X PM issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into 100X PM.
- GitHub webhooks update status/comment state in 100X PM.
- A PR is linked back to the 100X PM issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@100x-pm/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@100x-pm/plugin-grafana`

This plugin surfaces external metrics and dashboards inside 100X PM. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating 100X PM issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full 100X PM plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the 100X PM dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a 100X PM issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of 100X PM core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@100x-pm/plugin-runtime-processes`
...
Package idea: `@100x-pm/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into 100X PM. It is useful for:
...
- Failed payment spikes or churn events can generate 100X PM issues for follow-up.
...
- Stripe data should stay additive to 100X PM core
...
- plugin UI bundle loading, host bridge, `@100x-pmai/plugin-sdk/ui`
...
- `@100x-pmai/plugin-test-harness` and `create-100x-pm-plugin` starter template
...
100X PM should not implement "an OpenCode-style generic in-process hook system."
100X PM should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In 100X PM
...
8. Ship `@100x-pmai/plugin-test-harness` and `create-100x-pm-plugin` starter template.
