# 100X PM Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for 100X PM's plugin and extension architecture.
...
100X PM plugin design is based on the following assumptions:

1. 100X PM is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core 100X PM business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by 100X PM core.
...
2. Let plugins add major capabilities without editing 100X PM core.
...
4. Depend on project-local plugin folders such as `.100x-pm/plugins`.
...
The single 100X PM deployment an operator installs and controls.

### 5.2 Company

A first-class 100X PM business object inside the instance.
...
A trusted in-process extension loaded directly by 100X PM core.
...
An installable instance-wide extension package loaded through the 100X PM plugin runtime.
...
100X PM has two extension classes.
...
100X PM already has a concrete workspace model:
...
Plugins live under the 100X PM instance directory.

Suggested layout:

- `~/.100x-pm/instances/default/plugins/package.json`
- `~/.100x-pm/instances/default/plugins/node_modules/`
- `~/.100x-pm/instances/default/plugins/.cache/`
- `~/.100x-pm/instances/default/data/plugins/<plugin-id>/`

The package install directory and the plugin data directory are separate.

## 8.2 Operator Commands

100X PM should add CLI commands:

- `pnpm 100x-pmai plugin list`
- `pnpm 100x-pmai plugin install <package[@version]>`
- `pnpm 100x-pmai plugin uninstall <plugin-id>`
- `pnpm 100x-pmai plugin upgrade <plugin-id> [version]`
- `pnpm 100x-pmai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@100x-pm/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface 100X PMPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that 100X PM agents can use during runs.
...
- 100X PM server starts one worker process per installed plugin
...
Receives one typed 100X PM domain event.
...
export function definePlugin(definition: PluginDefinition): 100X PMPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: 100X PMPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@100x-pmai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@100x-pmai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@100x-pmai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@100x-pmai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@100x-pmai/plugin-sdk/ui`
...
The `@100x-pmai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the 100X PM secret provider system rather than a plain text input
...
1. Core 100X PM data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing 100X PM objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual 100X PM product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if 100X PM later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing 100X PM secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm 100x-pmai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the 100X PM server. This is a normative requirement, not optional.
...
### 27.1 `@100x-pmai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@100x-pmai/plugin-test-harness";
...
For developing a plugin against a running 100X PM instance:

- The operator installs the plugin from a local path: `pnpm 100x-pmai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-100x-pm-plugin`) that scaffolds:

- `package.json` with correct `100x-pmPlugin` keys
...
- `@100x-pm/plugin-workspace-files`
- `@100x-pm/plugin-terminal`
- `@100x-pm/plugin-git`
- `@100x-pm/plugin-linear`
- `@100x-pm/plugin-github-issues`
- `@100x-pm/plugin-grafana`
- `@100x-pm/plugin-runtime-processes`
- `@100x-pm/plugin-stripe`
...
- `@100x-pmai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@100x-pmai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@100x-pmai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@100x-pmai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@100x-pmai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@100x-pmai/plugin-sdk/ui`
...
- `@100x-pmai/plugin-test-harness`
- `create-100x-pm-plugin` starter template
...
100X PM should not implement a generic in-process hook bag modeled directly after local coding tools.

100X PM should implement:
...
That is the complete target design for the 100X PM plugin system.
