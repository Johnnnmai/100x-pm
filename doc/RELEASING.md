...
Maintainer runbook for shipping a full 100X PM release across npm, GitHub, and the website-facing changelog surface.
...
2. **npm** — `100x-pmai` and public workspace packages are published
...
```bash
VERSION=X.Y.Z
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for 100X PM. Read doc/RELEASING.md and skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
100X PM uses one release branch per target stable version:
...
./scripts/release-start.sh minor --worktree-dir ../100x-pm-release-0.4.0
...
- `npx 100x-pmai@canary onboard` installs
...
- [ ] `npm view 100x-pmai@latest version` matches the new stable version
