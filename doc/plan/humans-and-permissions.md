...
- local mode that still feels instant (`npx 100x-pmai run` and go)
...
2. CLI command `pnpm 100x-pmai auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm 100x-pmai onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm 100x-pmai onboard`).
...
- expose key instance settings in API + CLI (`100x-pmai configure` / `100x-pmai onboard`)
...
7. `pnpm 100x-pmai onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
