# 100X PM Specification

Target specification for the 100X PM control plane. Living document — updated incrementally during spec interviews.

---

## 1. Company Model [DRAFT]

A Company is a first-order object. One 100X PM instance runs multiple Companies. A Company does not have a standalone "goal" field — its direction is defined by its set of Initiatives (see Task Hierarchy Mapping).
...
Concepts like SOUL.md (identity/mission) and HEARTBEAT.md (loop definition) are **not part of the 100X PM protocol**. They are adapter-specific configurations. For example, an OpenClaw adapter might use SOUL.md and HEARTBEAT.md files. A Claude Code adapter might use CLAUDE.md. A bare Python script might use command-line args.

100X PM doesn't prescribe how an agent defines its identity or behavior. It provides the control plane; the adapter defines the agent's inner workings.
...
#### 100X PM Protocol (What 100X PM Knows)

At the protocol level, 100X PM tracks:
...
- **Fat payload** — 100X PM bundles relevant context (current tasks, messages, company state, metrics) into the heartbeat invocation. Suited for simple/stateless agents that can't call back to 100X PM.
- **Thin ping** — Heartbeat is just a wake-up signal. Agent calls 100X PM's API to fetch whatever context it needs. Suited for sophisticated agents that manage their own state.

#### Minimum Contract

The minimum requirement to be a 100X PM agent: **be callable.** That's it. 100X PM can invoke you via command or webhook. No requirement to report back — 100X PM infers basic status from process liveness when it can.

#### Integration Levels

Beyond the minimum, 100X PM provides progressively richer integration:

1. **Callable** (minimum) — 100X PM can start you. That's the only contract.
...
100X PM ships **default agents** that demonstrate full integration: progress tracking, cost instrumentation, and a **100X PM skill** (a Claude Code skill for interacting with the 100X PM API) for task management. These serve as both useful defaults and reference implementations for adapter authors.
...
Agents can create tasks and assign them to agents outside their reporting line. This is the mechanism for cross-team collaboration. These rules are primarily encoded in the 100X PM SKILL.md which is recommended for all agents. 100X PM the app enforces the tooling and some light governance, but the cross-team rules below are mainly implemented by agent decisions.
...
The heartbeat is a protocol, not a runtime. 100X PM defines how to initiate an agent's cycle. What the agent does with that cycle — how long it runs, whether it's task-scoped or continuous — is entirely up to the agent.

### Execution Adapters

Agent configuration includes an **adapter** that defines how 100X PM invokes the agent. Initial adapters:
...
This is the full adapter contract. `invoke` starts the agent, `status` lets 100X PM check on it, `cancel` enables the board's pause functionality. Everything else (cost reporting, task updates) is optional and flows through the 100X PM REST API.

### What 100X PM Controls
...
- Health monitoring — how does 100X PM distinguish "stuck" from "working on a long task"?
...
Token/LLM cost budgeting is a core part of 100X PM. External revenue and expense tracking is a future plugin.

### Cost Reporting

Fully-instrumented Agents report token/API usage back to 100X PM. Costs are tracked at every level:
...
100X PM ships default Agent templates:

- **Default Agent** — a basic Claude Code or Codex loop. Knows the **100X PM Skill** (SKILL.md) so it can interact with the task system, read Company context, report status.
...
### 100X PM Skill (SKILL.md)

A skill definition that teaches agents how to interact with 100X PM. Provides:
...
The key constraint: it must be trivial to go from "I'm trying this on my machine" to "my agents are running on remote servers talking to my 100X PM instance."

#### Agent Authentication

When a user creates an Agent, 100X PM generates a **connection string** containing: the server URL, an API key, and instructions for how to authenticate. The Agent is assumed to be capable of figuring out how to call the API with its token/key from there.

Flow:

1. Human creates an Agent in the UI
2. 100X PM generates a connection string (URL + key + instructions)
...
When a human completes a task, if the requesting agent's adapter supports **pingbacks** (e.g. OpenClaw hooks), 100X PM sends a notification to wake that agent. This keeps humans rare but possible participants in the workflow.

The agents are discouraged from assigning tasks to humans in the 100X PM SKILL, but sometimes it's unavoidable.
...
100X PM does **not** manage work artifacts (code repos, file systems, deployments, documents). That's entirely the agent's domain. 100X PM tracks tasks and costs. Where and how work gets done is outside scope.
...
When an agent crashes or disappears mid-task, 100X PM does **not** auto-reassign or auto-release the task. Instead:

- 100X PM surfaces stale tasks (tasks in `in_progress` with no recent activity) through dashboards and reporting
- 100X PM does not fail silently — the auditing and visibility tools make problems obvious
...
**Principle: 100X PM reports problems, it doesn't silently fix them.** Automatic recovery hides failures. Good visibility lets the right entity (human or agent) decide what to do.

### Plugin / Extension Architecture

The core 100X PM system must be extensible. Features like knowledge bases, external revenue tracking, and new Agent Adapters should be addable as **plugins** without modifying core. This means:
...
**Full loop with one adapter.** V1 must demonstrate the complete 100X PM cycle end-to-end, even if narrow.
...
- [ ] **Default agent** — basic Claude Code/Codex loop with 100X PM skill
- [ ] **Default CEO** — strategic planning, delegation, board communication
- [ ] **100X PM skill (SKILL.md)** — teaches agents to interact with the API
...
**Anti-goal for core.** The knowledge base is not part of the 100X PM core — it will be a plugin. The task system + comments + agent descriptions provide sufficient shared context.
...
Things 100X PM explicitly does **not** do:

- **Not an Agent runtime** — 100X PM orchestrates, Agents run elsewhere
...
1. **Unopinionated about how you run your Agents.** Any language, any framework, any runtime. 100X PM is the control plane, not the execution plane.
