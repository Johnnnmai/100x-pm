...
ClipHub is the public registry where people share, discover, and download 100X PM company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to 100X PM what a package registry is to a programming language. 100X PM already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in 100X PM — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own 100X PM instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the 100X PM spec. It contains:
...
**Install (fresh start):**
```
100x-pm install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local 100X PM instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within 100X PM, export your company as a template, then publish:

```
100x-pm export --template my-company
100x-pm publish cliphub my-company
```
...
ClipHub is a **separate service** from 100X PM itself. 100X PM is self-hosted; ClipHub is a hosted registry that 100X PM instances talk to.
...
| **100X PM CLI** | `100x-pmai install`, `100x-pmai publish`, `100x-pmai cliphub sync` — built into 100X PM |
| **100X PM UI** | "Browse ClipHub" panel in the 100X PM web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with 100X PM) |
| Backend | TypeScript + Hono (consistent with 100X PM) |
| Database | PostgreSQL |
...
4. Run `100x-pmai install cliphub:acme/lean-saas-shop`
5. 100X PM creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in 100X PM until it works well
2. Export: `100x-pmai export --template my-agency`
3. Publish: `100x-pmai publish cliphub my-agency`
...
3. Install just that agent: `100x-pmai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to 100X PM

ClipHub is **not required** to use 100X PM. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to 100X PM what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `100x-pmai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in 100X PM web UI
- [ ] `100x-pmai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with 100X PM's own philosophy)
