...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.100x-pm/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.100x-pm/instances/default/data/storage` (`local_disk`)
...
- exactly one JSON entrypoint: `100x-pm.manifest.json`
