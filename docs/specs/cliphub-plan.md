# ClipHub: Marketplace for 100X PM Team Configurations

> The "app store" for whole-company AI teams — pre-built 100X PM configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for 100X PM-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | 100X PM company (agents, projects, governance) |
...
A complete 100X PM company configuration:
...
Single-agent configurations designed to plug into a 100X PM org:
...
Portable skill files that any 100X PM agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with 100X PM's skill loading system
...
interface Listing {
  ...
  100x-pmVersionMin: string;     // Minimum 100X PM version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target 100X PM company
...
| `POST` | `/api/listings/:id/install` | Install to 100X PM company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target 100X PM company (or create new)
  → ClipHub API calls 100X PM API to:
  ...
  → Redirect to 100X PM dashboard with new team running
```
...
### 5.3 Creator: Export from 100X PM → Publish

```
Running 100X PM company → "Export as Blueprint" (CLI or UI)
  → 100X PM exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (100X PM brand blue/purple)
...
| Install flow | One-click deploy to 100X PM company |
...
{
  "targetCompanyId": "uuid",         // Existing 100X PM company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `100x-pm-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as 100X PM
- **Backend**: Node.js API (or extend 100X PM server)
- **Database**: Postgres (can share 100X PM's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with 100X PM auth (or OAuth2)

### 9.2 Integration with 100X PM

ClipHub can be:
- **Option A**: A separate app that calls 100X PM's API to install blueprints
- **Option B**: A built-in section of the 100X PM UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing 100X PM UI and API.
...
- [ ] Install flow: blueprint → 100X PM company
...
- [ ] "Export from 100X PM" CLI command
