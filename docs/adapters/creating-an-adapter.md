...
Build a custom adapter to connect 100X PM to any agent runtime.
...
2. Build environment with `build100X PMEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `100x-pmai run --watch` using `picocolors`.
...
Make 100X PM skills discoverable to your agent runtime without writing to the agent's working directory:
