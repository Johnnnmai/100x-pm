...
- Running a Python script that calls the 100X PM API
...
1. 100X PM spawns the configured command as a child process
2. Standard 100X PM environment variables are injected (`PAPERCLIP_AGENT_ID`, `PAPERCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the 100X PM API and perform work.
