...
Audience: Operators setting up and running agents in 100X PM

## 1. What this system does

Agents in 100X PM do not run continuously.  
...
For `claude_local` and `codex_local`, 100X PM assumes the CLI is already installed and authenticated on the host machine.
...
100X PM stores session IDs for resumable adapters.
...
100X PM pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. 100X PM surfaces this as a warning in environment tests, not a hard error.
