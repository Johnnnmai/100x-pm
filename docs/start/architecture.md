...
100X PM is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── 100x-pm/               # Core 100X PM skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with 100X PM env vars and a prompt
4. **Agent work** — The agent calls 100X PM's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between 100X PM and agent runtimes. Each adapter is a package with three modules:
...
- **CLI module** — terminal formatter for `100x-pmai run --watch`

Built-in adapters: `claude_local`, `codex_local`, `process`, `http`. You can create custom adapters for any runtime.

## Key Design Decisions

- **Control plane, not execution plane** — 100X PM orchestrates agents; it doesn't run them
