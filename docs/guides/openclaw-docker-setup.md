...
How to get OpenClaw running in a Docker container for local development and testing the 100X PM OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

100X PM includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-100x-pm-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a 100X PM host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-100x-pm-smoke`)
...
If your 100X PM deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host 100X PM server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed 100X PM URL (typically `http://host.docker.internal:3100`).
- If 100X PM rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm 100x-pmai allowed-hostname host.docker.internal
```

Then restart 100X PM and rerun the smoke script.
