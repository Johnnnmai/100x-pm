...
100X PM supports two runtime modes with different security profiles.
...
```sh
pnpm 100x-pmai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, 100X PM emits a one-time claim URL at startup:
...
```sh
PAPERCLIP_DEPLOYMENT_MODE=authenticated pnpm 100x-pmai run
```
