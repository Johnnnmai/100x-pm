...
100X PM uses PostgreSQL via Drizzle ORM. There are three ways to run the database.
...
1. Creates `~/.100x-pm/instances/default/db/` for storage
2. Ensures the `100x-pm` database exists
3. Runs migrations automatically
4. Starts serving requests

Data persists across restarts. To reset: `rm -rf ~/.100x-pm/instances/default/db`.
