...
function buildSourceConfig(): 100X PMConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("100x-pm-pr-432")).toBe(
      path.resolve(os.homedir(), "100x-pm-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("100x-pm/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://100x-pm.example", 3110)).toBe("https://100x-pm.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/100x-pm-feature",
      homeDir: "/tmp/100x-pm-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/100x-pm-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/100x-pm-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );

    const env = buildWorktreeEnvEntries(paths);
    expect(env.PAPERCLIP_HOME).toBe(path.resolve("/tmp/100x-pm-worktrees"));
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "100x-pm-worktree-secrets-"));
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "100x-pm-worktree-jwt-"));
    ...
    try {
      ...
      await worktreeInitCommand({
        ...
        home: path.join(tempRoot, ".100x-pm-worktrees"),
      });

      const envPath = path.join(repoRoot, ".100x-pm", ".env");
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/100x-pm",
        targetRepoRoot: "/Users/example/100x-pm-pr-432",
        workspaceCwd: "/Users/example/100x-pm",
      }),
    ).toBe("/Users/example/100x-pm-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/100x-pm",
        targetRepoRoot: "/Users/example/100x-pm-pr-432",
        workspaceCwd: "/Users/example/100x-pm/packages/db",
      }),
    ).toBe("/Users/example/100x-pm-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "100x-pm-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "100x-pm-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "100x-pm-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("100x-pm-make-test", {
        seed: false,
        home: path.join(tempRoot, ".100x-pm-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".100x-pm", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".100x-pm", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
