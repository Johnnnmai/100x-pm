...
describe("100X PMApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new 100X PMApiClient({
    ...
    expect(headers["x-100x-pm-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new 100X PMApiClient({ apiBase: "http://localhost:3100" });
