...
const DATA_DIR_OPTION_HELP =
  "100X PM data directory root (isolates state from ~/.100x-pm)";

program
  .name("100x-pmai")
  .description("100X PM CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  load100X PMEnvFile(options.config);
});
...
program
  ...
  .option("--run", "Start 100X PM immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your 100X PM setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "100x-pm")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run 100X PM")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the 100X PM server API")
