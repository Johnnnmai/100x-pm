...
export function deploymentAuthCheck(config: 100X PMConfig): CheckResult {
  ...
  if (mode === "local_trusted") {
    if (!isLoopbackHost(config.server.host)) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `100x-pmai configure --section server` and set host to 127.0.0.1",
      };
    }
    return {
      name: "Deployment/auth mode",
  ...
  if (!secret) {
    return {
      ...
      canRepair: false,
      repairHint: "Set BETTER_AUTH_SECRET before starting 100X PM",
    };
  }
  ...
  if (auth.baseUrlMode === "explicit" && !auth.publicBaseUrl) {
    return {
      ...
      canRepair: false,
      repairHint: "Run `100x-pmai configure --section server` and provide a base URL",
    };
  }
  ...
  if (exposure === "public") {
    if (auth.baseUrlMode !== "explicit" || !auth.publicBaseUrl) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `100x-pmai configure --section server` and select public exposure",
      };
    }
    try {
      ...
      return {
        ...
        canRepair: false,
        repairHint: "Run `100x-pmai configure --section server` and provide a valid URL",
      };
    }
  }
