...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# 100X PM environment variables",
    "# Generated by 100X PM CLI commands",
...
export function resolve100X PMEnvFile(configPath?: string): string {
...
export function load100X PMEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  merge100X PMEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function read100X PMEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function write100X PMEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function merge100X PMEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = read100X PMEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  write100X PMEnvEntries(next, filePath);
  return next;
}
