...
export async function doctor(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" 100x-pm doctor ")));
  ...
  try {
    ...
    const readResult: CheckResult = {
      ...
      canRepair: false,
      repairHint: "Run `100x-pmai configure --section database` or `100x-pmai onboard`",
    };
...
async function runRepairableCheck(input: {
  ...
  load100X PMEnvFile(input.configPath);
