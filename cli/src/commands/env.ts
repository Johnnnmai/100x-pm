...
const DEFAULT_AGENT_JWT_ISSUER = "100x-pm";
const DEFAULT_AGENT_JWT_AUDIENCE = "100x-pm-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolve100X PMInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolve100X PMInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" 100x-pm env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: 100X PMConfig | null = null;
...
function collectDeploymentEnvRows(config: 100X PMConfig | null, configPath: string): EnvVarRow[] {
