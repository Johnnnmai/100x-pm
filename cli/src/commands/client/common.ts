...
import { ApiRequestError, 100X PMApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to 100X PM config file")
    .option("-d, --data-dir <path>", "100X PM data directory root (isolates state from ~/.100x-pm)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the 100X PM API")
...
export function resolveCommandContext(
  ...
  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set PAPERCLIP_COMPANY_ID, or set context profile companyId via `100x-pmai context set`.",
    );
  }

  const api = new 100X PMApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
