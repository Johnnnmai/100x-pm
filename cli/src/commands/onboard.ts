...
type OnboardDefaults = Pick<100X PMConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolve100X PMInstanceId();
...
function canCreateBootstrapInviteImmediately(config: Pick<100X PMConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  print100X PMCliBanner();
  p.intro(pc.bgCyan(pc.black(" 100x-pmai onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolve100X PMInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: 100X PMConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@100x-pmai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `100x-pmai doctor`"));
      }
    }
  ...
  p.note(
    [
      `Run: ${pc.cyan("100x-pmai run")}`,
      `Reconfigure later: ${pc.cyan("100x-pmai configure")}`,
      `Diagnose setup: ${pc.cyan("100x-pmai doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("100x-pmai run")}`,
        `Then: ${pc.cyan("100x-pmai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
