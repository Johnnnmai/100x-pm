...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@100x-pmai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://100x-pm:100x-pm@127.0.0.1:${port}/100x-pm`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" 100x-pm db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolve100X PMInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "100x-pm";
