...
import { load100X PMEnvFile } from "../config/env.js";
...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  load100X PMEnvFile(configPath);

  p.intro(pc.bgCyan(pc.black(" 100x-pmai run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("100x-pmai onboard")} once, then retry ${pc.cyan("100x-pmai run")}.`);
      process.exit(1);
    }
  ...
  p.log.step("Starting 100X PM server...");
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  ...
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@100x-pmai/server/src/index.ts")) {
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @100x-pmai/server package
  try {
    const mod = await import("@100x-pmai/server");
    return await startServerFromModule(mod, "@100x-pmai/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@100x-pmai/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a 100X PM server entrypoint.\n` +
          `Tried: ${devEntry}, @100x-pmai/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `100X PM server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}

function shouldGenerateBootstrapInviteAfterStart(config: 100X PMConfig): boolean {
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`100X PM server entrypoint did not export startServer(): ${label}`);
  }
