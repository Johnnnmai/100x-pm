...
import { ensureAgentJwtSecret, load100X PMEnvFile, merge100X PMEnvEntries, read100X PMEnvEntries, resolve100X PMEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { 100X PMConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { print100X PMCliBanner } from "../utils/banner.js";
...
function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.100x-pm"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: 100X PMConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://100x-pm:100x-pm@127.0.0.1:${port}/100x-pm`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: 100X PMConfig;
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: 100X PMConfig;
  targetConfig: 100X PMConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolve100X PMEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = read100X PMEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://100x-pm:100x-pm@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "100x-pm");
    const targetConnectionString = `postgres://100x-pm:100x-pm@127.0.0.1:${targetHandle.port}/100x-pm`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = read100X PMEnvEntries(resolve100X PMEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.PAPERCLIP_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.PAPERCLIP_AGENT_JWT_SECRET);
  merge100X PMEnvEntries(
    {
  ...
  load100X PMEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run 100X PM inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  print100X PMCliBanner();
  p.intro(pc.bgCyan(pc.black(" 100x-pmai worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  print100X PMCliBanner();
  p.intro(pc.bgCyan(pc.black(" 100x-pmai worktree:make ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolve100X PMEnvFile(configPath);
  const envEntries = read100X PMEnvEntries(envPath);
  const out = {
    PAPERCLIP_CONFIG: configPath,
    ...(envEntries.100XPM_HOME ? { 100XPM_HOME: envEntries.100XPM_HOME } : {}),
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local 100X PM instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated 100X PM instance inside it")
    ...
    .option("--from-data-dir <path>", "Source 100XPM_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local 100X PM instance")
