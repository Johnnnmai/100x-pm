...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@100x-pmai/db";
import type { DeploymentMode } from "@100x-pmai/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  100x-pmUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).100x-pmUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.100x-pmUpgradeContext = context;
