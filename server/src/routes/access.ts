...
import type { Db } from "@100x-pmai/db";
import {
  agentApiKeys,
  authUsers,
  invites,
...
import type { DeploymentExposure, DeploymentMode } from "@100x-pmai/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (normalized !== "100x-pm" && normalized !== "100x-pm-create-agent")
    return null;
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.100x-pmApiUrl)) {
    const legacy100X PMApiUrl = nonEmptyTrimmedString(input.100x-pmApiUrl);
    if (legacy100X PMApiUrl) merged.100x-pmApiUrl = legacy100X PMApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    100x-pmApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.100x-pmApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const raw100X PMApiUrl =
    typeof defaults.100x-pmApiUrl === "string"
      ? defaults.100x-pmApiUrl.trim()
      : "";
  if (raw100X PMApiUrl) {
    try {
      const parsed100X PMApiUrl = new URL(raw100X PMApiUrl);
      if (
        parsed100X PMApiUrl.protocol !== "http:" &&
        parsed100X PMApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_100x-pm_api_url_protocol",
          level: "warn",
          message: `100x-pmApiUrl must use http:// or https:// (got ${parsed100X PMApiUrl.protocol}).`
        });
      } else {
        normalized.100x-pmApiUrl = parsed100X PMApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_100x-pm_api_url_configured",
          level: "info",
          message: `100x-pmApiUrl set to ${parsed100X PMApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_100x-pm_api_url_invalid",
        level: "warn",
        message: `Invalid 100x-pmApiUrl: ${raw100X PMApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your 100X PM host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "100X PM is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm 100x-pmai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/100x-pm";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/100x-pm-claimed-api-key.json and load PAPERCLIP_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: 100x-pmApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the 100X PM hostname is reachable and allowed via `pnpm 100x-pmai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this 100X PM API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/100x-pm/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # 100X PM OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          100x-pmApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, 100X PM generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "100x-pmApiUrl": "https://100x-pm-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in 100X PM before key claim is allowed.
    ...
    - ~/.openclaw/workspace/100x-pm-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/100x-pm-claimed-api-key.json
    ...
    ## Step 4: Install 100X PM skill in OpenClaw
    ...
    ${
      ...
      "Ensure 100X PM is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested 100X PM base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.100x-pmApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm 100x-pmai allowed-hostname <host>
      - then restart 100X PM and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@100x-pm.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "100x-pm", path: "/api/skills/100x-pm" },
        {
          name: "100x-pm-create-agent",
          path: "/api/skills/100x-pm-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              100x-pmApiUrl: req.body.100x-pmApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.100x-pmApiUrl &&
          !persistedDefaults.100x-pmApiUrl
        ) {
          missingPersistedFields.push("100x-pmApiUrl");
        }
