...
export async function startServer(): Promise<StartedServer> {
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@100x-pm.local";
  ...
  if (config.databaseUrl) {
    ...
    const embeddedAdminConnectionString = `postgres://100x-pm:100x-pm@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "100x-pm");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: 100x-pm");
    }
  
    const embeddedConnectionString = `postgres://100x-pm:100x-pm@127.0.0.1:${port}/100x-pm`;
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "100X PM server failed to start");
    process.exit(1);
  });
}
