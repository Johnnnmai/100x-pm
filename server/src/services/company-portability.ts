...
import { normalizeAgentUrlKey, portabilityManifestSchema } from "@100x-pmai/shared";
...
export function companyPortabilityService(db: Db) {
  ...
  async function resolveSource(source: CompanyPortabilityPreview["source"]): Promise<ResolvedSource> {
    ...
    let ref = parsed.ref;
    const manifestRelativePath = [parsed.basePath, "100x-pm.manifest.json"].filter(Boolean).join("/");
