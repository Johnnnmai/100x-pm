...
const DEFERRED_WAKE_CONTEXT_KEY = "_100x-pmWakeContext";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__100x-pm_repo_only__";
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    context.100x-pmWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.100x-pmRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.100x-pmRuntimeServiceIntents;
    }
    ...
    try {
      ...
      for (const warning of runtimeWorkspaceWarnings) {
        await onLog("stderr", `[100x-pm] ${warning}\n`);
      }
      ...
      if (runtimeServices.length > 0) {
        context.100x-pmRuntimeServices = runtimeServices;
        context.100x-pmRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[100x-pm] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.100x-pmRuntimeServices = combinedRuntimeServices;
        context.100x-pmRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[100x-pm] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
