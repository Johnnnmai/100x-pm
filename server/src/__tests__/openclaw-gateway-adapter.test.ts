...
import { execute, testEnvironment } from "@100x-pmai/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@100x-pmai/adapter-openclaw-gateway/ui";
import type { AdapterExecutionContext } from "@100x-pmai/adapter-utils";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("100x-pm:issue:issue-123");
