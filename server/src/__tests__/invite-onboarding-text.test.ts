...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("100X PM OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested 100X PM base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("100x-pmApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.100x-pmApiUrl");
    expect(text).toContain("~/.openclaw/workspace/100x-pm-claimed-api-key.json");
