...
import { ensureCursorSkillsInjected } from "@100x-pmai/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing 100X PM skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("100x-pm-cursor-skills-src-");
    const skillsHome = await makeTempDir("100x-pm-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "100x-pm");
    await createSkillDir(skillsDir, "100x-pm-create-agent");
    ...
    const injectedA = path.join(skillsHome, "100x-pm");
    const injectedB = path.join(skillsHome, "100x-pm-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "100x-pm")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "100x-pm-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "100x-pm"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "100x-pm-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("100x-pm-cursor-preserve-src-");
    const skillsHome = await makeTempDir("100x-pm-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "100x-pm");
    ...
    expect((await fs.lstat(path.join(skillsHome, "100x-pm-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("100x-pm-cursor-fail-src-");
    const skillsHome = await makeTempDir("100x-pm-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
