...
async function ensureEmbeddedPostgresConnection(
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://100x-pm:100x-pm@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "100x-pm");
    return {
      connectionString: `postgres://100x-pm:100x-pm@127.0.0.1:${port}/100x-pm`,
  ...
  const adminConnectionString = `postgres://100x-pm:100x-pm@127.0.0.1:${preferredPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "100x-pm");

  return {
    connectionString: `postgres://100x-pm:100x-pm@127.0.0.1:${preferredPort}/100x-pm`,
