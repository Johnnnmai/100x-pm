...
export const databaseBackupConfigSchema = z.object({
  ...
  dir: z.string().default("~/.100x-pm/instances/default/data/backups"),
});

export const databaseConfigSchema = z.object({
  mode: z.enum(["embedded-postgres", "postgres"]).default("embedded-postgres"),
  connectionString: z.string().optional(),
  embeddedPostgresDataDir: z.string().default("~/.100x-pm/instances/default/db"),
  embeddedPostgresPort: z.number().int().min(1).max(65535).default(54329),
  backup: databaseBackupConfigSchema.default({
    enabled: true,
    intervalMinutes: 60,
    retentionDays: 30,
    dir: "~/.100x-pm/instances/default/data/backups",
  }),
});

export const loggingConfigSchema = z.object({
  mode: z.enum(["file", "cloud"]),
  logDir: z.string().default("~/.100x-pm/instances/default/logs"),
});
...
export const storageLocalDiskConfigSchema = z.object({
  baseDir: z.string().default("~/.100x-pm/instances/default/data/storage"),
});

export const storageS3ConfigSchema = z.object({
  bucket: z.string().min(1).default("100x-pm"),
...
export const storageConfigSchema = z.object({
  ...
  localDisk: storageLocalDiskConfigSchema.default({
    baseDir: "~/.100x-pm/instances/default/data/storage",
  }),
...
export const secretsLocalEncryptedConfigSchema = z.object({
  keyFilePath: z.string().default("~/.100x-pm/instances/default/secrets/master.key"),
});
...
export const secretsConfigSchema = z.object({
  ...
  localEncrypted: secretsLocalEncryptedConfigSchema.default({
    keyFilePath: "~/.100x-pm/instances/default/secrets/master.key",
  }),
});

export const 100x-pmConfigSchema = z
  .object({
...
export type 100X PMConfig = z.infer<typeof 100x-pmConfigSchema>;
