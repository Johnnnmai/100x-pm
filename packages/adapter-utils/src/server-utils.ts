...
export function build100X PMEnv(agent: { id: string; companyId: string }): Record<string, string> {
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the 100X PM server itself is started from
    // within a Claude Code session (e.g. `npx 100x-pmai run` in a terminal
