...
Use when:
- You want 100X PM to run Pi (the AI coding agent) locally as the agent runtime
...
- 100X PM requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/100x-pms/ and resumed with --session.
