...
Use when:
- You want 100X PM to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the 100X PM server.
...
- workspaceRuntime (object, optional): desired runtime service intents; 100X PM forwards these in a standardized 100x-pm.workspaceRuntime block for remote execution environments
- timeoutSec (number, optional): adapter timeout in seconds (default 120)
- waitTimeoutMs (number, optional): agent.wait timeout override (default timeoutSec * 1000)
- autoPairOnFirstConnect (boolean, optional): on first "pairing required", attempt device.pair.list/device.pair.approve via shared auth, then retry once (default true)
- 100x-pmApiUrl (string, optional): absolute 100X PM base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default 100x-pm)

Standard outbound payload additions:
- 100x-pm (object): standardized 100X PM context added to every gateway agent request
- 100x-pm.workspace (object, optional): resolved execution workspace for this run
- 100x-pm.workspaces (array, optional): additional workspace hints 100X PM exposed to the run
- 100x-pm.workspaceRuntime (object, optional): normalized runtime service intent config for the workspace
