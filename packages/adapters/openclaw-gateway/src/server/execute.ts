...
import { asNumber, asString, build100X PMEnv, parseObject } from "@100x-pmai/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "100x-pm";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "100x-pm";
  if (input.strategy === "run") return `100x-pm:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `100x-pm:issue:${input.issueId}`;
  return fallback;
}
...
function resolve100X PMApiUrlOverride(value: unknown): string | null {
...
function build100X PMEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const 100x-pmApiUrlOverride = resolve100X PMApiUrlOverride(ctx.config.100x-pmApiUrl);
  const 100x-pmEnv: Record<string, string> = {
    ...build100X PMEnv(ctx.agent),
    PAPERCLIP_RUN_ID: ctx.runId,
  };

  if (100x-pmApiUrlOverride) {
    100x-pmEnv.PAPERCLIP_API_URL = 100x-pmApiUrlOverride;
  }
  if (wakePayload.taskId) 100x-pmEnv.PAPERCLIP_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) 100x-pmEnv.PAPERCLIP_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) 100x-pmEnv.PAPERCLIP_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) 100x-pmEnv.PAPERCLIP_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) 100x-pmEnv.PAPERCLIP_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    100x-pmEnv.PAPERCLIP_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return 100x-pmEnv;
}

function buildWakeText(payload: WakePayload, 100x-pmEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/100x-pm-claimed-api-key.json";
  const orderedKeys = [
    "PAPERCLIP_RUN_ID",
    "PAPERCLIP_AGENT_ID",
  ...
  for (const key of orderedKeys) {
    const value = 100x-pmEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = 100x-pmEnv.PAPERCLIP_API_URL ?? "<set PAPERCLIP_API_URL>";

  const lines = [
    "100X PM wake event for a cloud adapter.",
    "",
    ...
    "- Use X-100X PM-Run-Id: $PAPERCLIP_RUN_ID on every mutating API call.",
...
function buildStandard100X PMPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  100x-pmEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const template100X PM = parseObject(payloadTemplate.100x-pm);
  const workspace = asRecord(ctx.context.100x-pmWorkspace);
  const workspaces = Array.isArray(ctx.context.100x-pmWorkspaces)
    ? ctx.context.100x-pmWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.100x-pmRuntimeServiceIntents)
    ? ctx.context.100x-pmRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standard100X PM: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: 100x-pmEnv.PAPERCLIP_API_URL ?? null,
  };

  if (workspace) {
    standard100X PM.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standard100X PM.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standard100X PM.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "100x-pm-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const 100x-pmEnv = build100X PMEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, 100x-pmEnv);
  ...
  const 100x-pmPayload = buildStandard100X PMPayload(ctx, wakePayload, 100x-pmEnv, payloadTemplate);
