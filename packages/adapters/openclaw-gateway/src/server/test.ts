...
import { asString, parseObject } from "@100x-pmai/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the 100X PM server host.",
        });
      }
    } catch (err) {
      checks.push({
