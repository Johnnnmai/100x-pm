...
## Scope
This plan is now **gateway-only**. 100X PM supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses 100X PM skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a 100X PM task.
...
```bash
pnpm --filter @100x-pmai/server test:run -- openclaw-gateway
pnpm --filter @100x-pmai/server typecheck
pnpm --filter @100x-pmai/ui typecheck
pnpm --filter 100x-pmai typecheck
```
