...
Use when:
- You want 100X PM to run Cursor Agent CLI locally as the agent runtime
...
- 100X PM auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$100x-pm" and related skills on local runs.
- 100X PM auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
