...
description: >
  Technical guide for creating a new 100X PM agent adapter. Use when building
...
# Creating a 100X PM Agent Adapter

An adapter bridges 100X PM's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `100x-pmai run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@100x-pmai/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@100x-pmai/adapter-utils` (types) or `@100x-pmai/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell 100X PM to forget the stored session
}
...
```json
{
  ...
  "dependencies": {
    "@100x-pmai/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
The `agentConfigurationDoc` is read by LLM agents (including 100X PM agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@100x-pmai/adapter-utils/server-utils`)
2. **Build environment** — call `build100X PMEnv(agent)` then layer in `PAPERCLIP_RUN_ID`, context vars (`PAPERCLIP_TASK_ID`, `PAPERCLIP_WAKE_REASON`, `PAPERCLIP_WAKE_COMMENT_ID`, `PAPERCLIP_APPROVAL_ID`, `PAPERCLIP_APPROVAL_STATUS`, `PAPERCLIP_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
Pretty-prints stdout lines for `100x-pmai run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@100x-pmai/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@100x-pmai/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use 100X PM API
...
import { parseMyAgentStdoutLine } from "@100x-pmai/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@100x-pmai/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@100x-pmai/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so 100X PM wipes the stale session
...
Import from `@100x-pmai/adapter-utils/server-utils`:
...
| `build100X PMEnv(agent)` | Standard `PAPERCLIP_*` env vars |
...
- Package name: `@100x-pmai/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your 100X PM work."`
...
### 100X PM Skills Injection

100X PM ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `100x-pm` API skill and the `100x-pm-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with 100X PM internals, break git status, and potentially leak into commits.

**The pattern:** create a clean, isolated location for skills and tell the agent runtime to look there.

**How claude-local does it:**

1. At execution time, create a fresh tmpdir: `mkdtemp("100x-pm-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "100x-pm-skills-"));
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks 100X PM skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
**Skills as loaded procedures, not prompt bloat.** The 100X PM skills (like `100x-pm` and `100x-pm-create-agent`) are designed as on-demand procedures: the agent sees skill metadata (name + description) in its context, but only loads the full SKILL.md content when it decides to invoke a skill. This keeps the base prompt small. When writing `agentConfigurationDoc` or prompt templates for your adapter, do not inline skill content — let the agent runtime's skill discovery do the work. The descriptions in each SKILL.md frontmatter act as routing logic: they tell the agent when to load the full skill, not what the skill contains.

**Explicit vs. fuzzy skill invocation.** For production workflows where reliability matters (e.g. an agent that must always call the 100X PM API to report status), use explicit instructions in the prompt template: "Use the 100x-pm skill to report your progress." Fuzzy routing (letting the model decide based on description matching) is fine for exploratory tasks but unreliable for mandatory procedures.

---

## 8. Security Considerations

Adapters sit at the boundary between 100X PM's orchestration layer and arbitrary agent execution. This is a high-risk surface.
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the 100X PM API and GitHub should not have access to arbitrary hosts.
